...
{//发送消息
    if(ui->lineEdit_sendmessage->text()!="")
    {
        ...
        {//服务器连接成功
            //第一个是时间，第二个发送的ID，第三个是接受的ID
            QString nowstr = QDateTime::currentDateTime().currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
            QString message = QString("chat_send##%1##%2##%3##%4##%5").arg(nowstr).arg(user.id).arg(otheruser.id).arg(user.ip).arg(ui->lineEdit_sendmessage->text());
        ...
        connect(tcpSocket,&QTcpSocket::readyRead,[=](){
            ...
            if(QString(buffer).section("##",0,0)==QString("chat_history_ok"))
            {
                ...
                for(int rownum = 0;rownum < num ;rownum++)
                {
                    ...
                    qDebug()<<timeshow;
                    QString idshow = "";
                    if(QString(buffer).section("##",rownum*4+3,rownum*4+3).toInt()==user.id)
                    {//我自己发送的消息
                        idshow = " 我 "+QString(buffer).section("##",rownum*4+4,rownum*4+4)+" :";
                    }
                    else
                    {
                        idshow =" "+ otheruser.name + " "+QString(buffer).section("##",rownum*4+4,rownum*4+4)+" ：";
                    }
                    chatshow = "("+timeshow+")" + idshow + QString(buffer).section("##",rownum*4+5,rownum*4+5) +"\n" + chatshow;
                }
