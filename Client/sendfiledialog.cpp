...
void sendfiledialog::acceptConnection(){
    int sendport = 9988;//开放端口接受连接
    tcpServer = new QTcpServer(this);
    tcpServer->listen(QHostAddress::Any,sendport);
    connect(tcpServer,&QTcpServer::newConnection,this,&sendfiledialog::getConnection);
    ui->pushButton_sendfile->setEnabled(false);
}

void sendfiledialog::getConnection(){
    ...
    connect(tcpSocket,&QTcpSocket::readyRead,this,&sendfiledialog::confirmSend);
}

void sendfiledialog::confirmSend(){
    QByteArray buffer = tcpSocket->readAll();
    qDebug()<<buffer;
    if("send_ok" == QString(buffer)){this->close();}
}
